...
for($rodada=1;$rodada<=38;$rodada++)
{
	...
	*/
	...
	foreach($dados['jogos'] as $jogo => $v)
	{
		...
		$ext = ".jhtm";
		$nome = $time1."-x-".$time2."-".$dados['jogos'][$jogo]['datajogo'][2]."-".$dados['jogos'][$jogo]['datajogo'][1];
				
		$linkJogo = $EstatisticasURL.$nome.$ext;
	...
	$dados = substr($dados, $inicio, ($tam-$inicio-2));
